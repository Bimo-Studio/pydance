...
class WrapFont:
    ...
    def lines(self, text, indent=""):
        ...
        for i in range(len(words)):
            if start == 0:
                ...
                line = indent + " ".join(words[start : i + 1])
    ...
    def render(self, text, color=[255, 255, 255], shdw=True, indent="", centered=False):
        lines = []
        words = text.split()
        start = 0
        for i in range(len(words)):
            ...
            if self._font.size(line)[0] > self._width:
                if start == 0:
                    ...
                    line = indent + " ".join(words[start:i])
                if shdw:
                    t = shadow(line, self._font, color)
                else:
                    t = self._font.render(line, True, color)
                lines.append(t)
                start = i
        if start == 0:
            line = " ".join(words[start:])
        else:
            line = indent + " ".join(words[start:])
        if shdw:
            t = shadow(line, self._font, color)
        else:
            t = self._font.render(line, True, color)
        ...
        image = pygame.Surface([self._width, len(lines) * self._ls + self._ds], SRCALPHA, 32)
        image.fill([0, 0, 0, 0])
...
class TextZoomer:
    def __init__(self, text, font, size, fore, back):
        self.tempsurface = pygame.surface.Surface(size, SRCALPHA, 32)
        self.back = back
        self.fore = fore
        self.size = size
        self.font = font
    ...
    def iterate(self):
        ...
        zoomsurface = pygame.transform.scale(
            self.tempsurface, [self.size[0] + 16, self.size[1] + 16]
        )
