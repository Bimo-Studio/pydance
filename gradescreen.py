...
class GrooveGaugeSprite(pygame.sprite.Sprite):
    def __init__(self, pos, size, records):
        ...
        for i in range(width):
            ...
            if plife > 0.5:
                c = colors.average(c1, c2, abs((plife - 0.5) / 0.5))
            else:
                c = colors.average(c2, c3, abs(plife / 0.5))
            pygame.draw.line(self._image, c, [i, size[1] - 1], [i, h])
            if plife > 0.999999:
                self._image.set_at([i, 0], [255, 255, 255])
...
class StatSprite(pygame.sprite.Sprite):
    ...
    def _render(self):
        ...
        cnt = fontfx.shadow(
            locale.format_string("%d", self._curcount, True),
            FontTheme.GrScr_text,
            colors.WHITE,
        )
        rc = cnt.get_rect()
    ...
    def update(self, time):
        if time < self._start:
            ...
            self._curcount = min(int(self._count * ((time - self._start) / 1000.0)), self._count)
            self._render()
...
class GradingScreen(InterfaceWindow):
    def __init__(self, screen, players, banner_fn):
        ...
        if self.players[0] is None:
            return None
        elif self.players[0].stats.arrow_count == 0:
            return None
        ...
        self._sprites.add(
            [
                ...
                HoldStatSprite(
                    [400, 39],
                    _("Holds:"),
                    plr.stats.good_holds,
                    plr.stats.hold_count,
                    s,
                    1000,
                ),
        ...
        self._sprites.add(GrooveGaugeSprite([10, 22], [176, 100], plr.lifebar.record))
        ...
        if len(self.players) == 2:
            ...
            self._sprites.add(GrooveGaugeSprite([453, 370], [176, 100], plr.lifebar.record))
        ...
        while ev not in exits:
            ...
            # all the animations.
                ...
                ev == ui.CONFIRM
                or ev == ui.OPTIONS
                or (pygame.time.get_ticks() - start > 3333 and not self._time_bonus)
            ):
