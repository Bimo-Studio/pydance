...
DESCRIPTIONS = {
    _("4 panel"): (
        _("The standard up, down, left and right arrows ") + _("(like Dance Dance Revolution)")
    ),
    ...
    _("DMX"): (
        _("Crazy kung-fu action (like Dance ManiaX / Freaks). ")
...
class MainWindow(InterfaceWindow):
    def __init__(self, songs, courses, screen):
        ...
        self._lists = [
            ListBox(FontTheme.GameSel_list, MENU_NAV_TEXT, 26, 9, 220, [408, 53]),
            ListBox(FontTheme.GameSel_list, MENU_NAV_TEXT, 26, 3, 220, [408, 300]),
            ListBox(FontTheme.GameSel_list, MENU_NAV_TEXT, 26, 3, 220, [408, 393]),
        ]
    ...
    def loop(self):
        ...
        while not (ev == ui.CANCEL and active == 0):
            if ev == ui.MOUSE_CLICK:
                ...
                hit = self._lists[active].item_index_at_screen_pos(mx, my)
                if hit is not None:
                    indices[active] = hit
                    self._lists[active].set_index(indices[active], 1)
                    text = VALUES[active][indices[active]]
                    self._selected.set_text(text)
                    self._description.set_text(DESCRIPTIONS[text])
                    img = IMAGES.get(text)
                    if callable(img):
                        self._image.set_image(img(self._oldimage))
                    else:
            ...
            if list_ud:
                ...
                if active == 2:
                    ...
                    SELECTORS[SS[indices[2]]](
                        self._songs,
                        self._courses,
                        self._screen,
                        MODES.get((VALUES[0][indices[0]], VALUES[1][indices[1]])),
                    )
                    active = 0
                    self._screen.blit(self._bg, [0, 0])
                    pygame.display.update()
                    ...
                    active += 1
                    if active == 1:
                        self._oldimage = self._image._image  # FIXME
