...
OPTIONS = {
    ...
    "speed": (
        ...
        list(
            zip(
                ...
                [""] * 12
                + [
                    _("Arrows scroll at 200bpm."),
                    _("Arrows scroll at 300bpm."),
                    _("Arrows scroll at 400bpm."),
                    _("Arrows scroll at 500bpm."),
                    _("Arrows scroll at 600bpm."),
                    _("Arrows scroll at 700bpm."),
                    _("Arrows scroll at 800bpm."),
                ],
            )
        ),
    ),
    ...
    "spin": (True, _("Spin"), _("Rotate arrows as they go up the screen."), ON_OFF),
    "colortype": (
        True,
        _("Colors"),
    ...
    "dark": (True, _("Dark"), _("Don't display the top arrows."), ON_OFF),
    "holds": (True, _("Holds"), _("Allow hold ('freeze') arrows in songs."), ON_OFF),
    "scoring": (False, _("Scoring"), _("The scoring algorithm."), scores.score_opt),
    "grade": (False, _("Grades"), _("The grading algorithm."), grades.grade_opt),
    "combo": (False, _("Combos"), _("How your combo increases."), combos.combo_opt),
    "judge": (
        False,
        _("Judging Method"),
        ...
        list(
            zip(
                ...
                [
                    ...
                    _("Window == 9/5 normal size."),
                    _("Window == 8/5 normal size."),
                    _("Window == 7/5 normal size."),
                    _("Window == 6/5 normal size."),
                    "",
                    _("Window == 4/5 normal size."),
                    _("Window == 3/5 normal size."),
                    _("Window == 2/5 normal size."),
                    _("Window == 1/5 normal size."),
                ],
            )
        ),
    ),
    "life": (
        ...
        [
            (0.25, _("Undead"), _("Life == 1/4 the usual amount.")),
            (0.50, _("Very Low"), _("Life == 1/2 the usual amount.")),
            (0.75, _("Low"), _("Life == 3/4 the usual amount.")),
            (1, _("Normal"), ""),
            (1.25, _("High"), _("Life == 5/4 the usual amount.")),
            (1.50, _("Very High"), _("Life == 3/2 the usual amount.")),
            (1.75, _("Lazarus"), _("Life == 7/4 the usual amount.")),
        ],
    ),
    "lifebar": (
        False,
...
class OptionSelect(pygame.sprite.Sprite):
    ...
    def _render(self, pct):
        ...
        if pct <= 0:
            ...
            offset = (
                self._font.size(self._possible[self._oldindex])[0] // 2
                + self._font.size(self._possible[self._index])[0] // 2
                + 30
            )
            offset = int(pct * offset)
            if self._oldindex > self._index:
                offset = -offset
        else:
            offset = 0

        t = fontfx.shadow(
            self._possible[self._index],
            FontTheme.Opts_choices,
            MENU_NAV_TEXT,
...
class OptionScreen(InterfaceWindow):
    def __init__(self, player_configs, game_config, screen, whitelist=None):
        ...
        self._lists = [ListBox(FontTheme.Opts_list, MENU_NAV_TEXT, 25, 9, 176, [10, 10])]
        self._text = [
            WrapTextDisplay(
                FontTheme.Opts_description,
                430,
                [198, 165],
                centered=True,
        ...
        self._text2 = [
            WrapTextDisplay(
                FontTheme.Opts_choice_description,
                430,
                [198, 105],
                centered=True,
                str=desc,
        ...
        self._displayers = [OptionSelect(names, [415, 40], index_of(val, self.optlist[0]))]
        self._index = [0]
        ...
        for lst in self._lists:
            lst.set_items([OPTIONS[k][1] for k in self.optlist])
        ...
        while ev not in [ui.OPTIONS, ui.CANCEL, ui.CONFIRM]:
            if pid >= self._players:
                ...
                if opt == "__start__":
                    break
                if OPTIONS[opt][PP]:
                    index = index_of(self._configs[pid][opt], opt)
                else:
                    index = index_of(self._config[opt], opt)
                if index > 0:
                    index -= 1
                if OPTIONS[opt][PP]:
                    self._configs[pid][opt] = value_of(index, opt)
                else:
                    self._config[opt] = value_of(index, opt)
                ...
                if OPTIONS[opt][PP]:
                    ...
                    index = index_of(self._config[opt], opt)
                if index != len(OPTIONS[opt][VALUES]) - 1:
                    index += 1
                if OPTIONS[opt][PP]:
                    self._configs[pid][opt] = value_of(index, opt)
                else:
