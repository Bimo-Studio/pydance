...
def difficulty_sort(a, b):
    if a in DIFFICULTY_LIST and b in DIFFICULTY_LIST:
        return (DIFFICULTY_LIST.index(a)) - (
            DIFFICULTY_LIST.index(b) if isinstance(DIFFICULTY_LIST.index(a), (int, float)) else 0
        )
    elif a in DIFFICULTY_LIST:
        return -1
    elif b in DIFFICULTY_LIST:
        return 1
    else:
...
def difficulty_sort_key(k):
    try:
        return DIFFICULTY_LIST.index(k)
    except ValueError:
        return len(DIFFICULTY_LIST)
...
def find_subtitle(title):
    for pair in [("[", "]"), ("(", ")"), ("~", "~"), ("-", "-")]:
        if pair[0] in title and title[-1] == pair[1]:
            open_idx = title[0:-1].rindex(pair[0])
            if open_idx != 0:
                subtitle = title[open_idx:]
                title = title[:open_idx]
...
def find(path, patterns, dedup_level):
    ...
    if not os.path.isdir(root):
        logger.warning("search path is not a directory (skipping): %s", root)
        return matches
...
def titlecase(title):
    ...
    vowels = "aeiouyAEIOUY"
    letters = string.ascii_letters + "?!'"  # Yeah, those aren't letters, but...
