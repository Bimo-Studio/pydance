...
# Legacy Makefile / pydance.cfg generator (was named setup.py).
# Run manually:  python configure_install.py
# For pip / editable installs use:  pip install -e ".[dev]"
...
def sanity_check():
    ...
    if sys.version_info < (3, 10):
        print("Versions of Python less than 3.10 are not supported by pydance.")
        print("Visit https://www.python.org to upgrade.")
        sys.exit(1)
    ...
    try:
        ...
        if pygame.version.vernum < (2, 0):
            print("You have pygame, but a version less than 2.0 is not supported.")
            print("Visit https://www.pygame.org to upgrade.")
            sys.exit(1)
    except ImportError:
