...
class AbstractLifeBar(Listener, pygame.sprite.Sprite):
    def __init__(self, playernum, maxlife, songconf, game):
        ...
        self.failtext = fontfx.embfade(
            _("FAILED"), FontTheme.Dance_lifebar_text, 3, [80, 32], [224, 32, 32]
        )
...
class LifeBarDisp(AbstractLifeBar):
    ...
    def draw(self, time):
        ...
        self.image.set_clip([0, 0, int(202 * self.displayed_life / self.maxlife), 28])
        self.image.blit(full, [0, 0])
        self.image.set_clip()

    def update(self, time):
        if self.gameover and self.displayed_life <= 0:
            return

        if self.displayed_life < self.life:
            self.displayed_life += 0.01
        elif self.displayed_life > self.life:
            self.displayed_life -= 0.01
        ...
        if self.life < 0:
            self.gameover = FAILED
        if self.displayed_life < 0:
            self.displayed_life = 0
...
class TugLifeBarDisp(LifeBarDisp):
    active_bars: list[Any] = []

    def __init__(self, playernum, theme, songconf, game):
        LifeBarDisp.__init__(self, playernum, theme, songconf, game)

        self.wontext = fontfx.embfade(
            "WON", FontTheme.Dance_lifebar_text, 3, (80, 32), (224, 32, 32)
        )
        ...
        if playernum == 0:
            TugLifeBarDisp.active_bars = [self]
        else:
            TugLifeBarDisp.active_bars.append(self)

    def stepped(self, pid, dir, curtime, etime, rating, combo):
        LifeBarDisp.stepped(self, pid, dir, curtime, etime, rating, combo)
        for bar in TugLifeBarDisp.active_bars:
            if bar != self:
                bar.update_life_opponent(rating)
    ...
    def update(self, time):
        ...
        if self.gameover == WON:
            ...
            for lifebar in TugLifeBarDisp.active_bars:
                if lifebar != self and not lifebar.gameover:
                    lifebar.gameover = WON
...
class MediocreLifeBarDisp(AbstractLifeBar):
    def __init__(self, playernum, theme, songconf, game):
        ...
        self.deltas = {"V": 0.04, "P": 0.04, "G": 0.02, "O": -0, "B": -0.02, "M": -0.04}
    ...
    def update(self, time):
        ...
        if self.life < 0 or self.life == self.maxlife:
            self.gameover = FAILED
