...
STEP_MAPPINGS = {
    ...
    "5PANEL": [[0, 1, 2, 3, 4], [3, 4, 2, 0, 1], [4, 0, 2, 1, 3], [1, 3, 2, 4, 5]],
    "PARAPARA": [[0, 1, 2, 3, 4], [1, 0, 2, 4, 3], [4, 0, 1, 2, 3], [1, 2, 3, 4, 0]],
    "6PANEL": [
        [0, 1, 2, 3, 4, 5],
        [4, 5, 3, 2, 0, 1],
        [2, 0, 5, 1, 3, 4],
    ...
    "EZ2SINGLE": [[0, 1, 2, 3, 4], [4, 3, 2, 1, 0], [4, 3, 2, 1, 0], [4, 3, 2, 1, 0]],
    "EZ2REAL": [
        [0, 1, 2, 3, 4, 5, 6],
        [6, 4, 5, 3, 1, 2, 0],
        [3, 2, 4, 6, 1, 5, 0],
...
for mode, equivs in MAP_EQUIVS.items():
    for eq in equivs:
        STEP_MAPPINGS[eq] = STEP_MAPPINGS[mode]
    ...
    for s in steps:
        if not isinstance(s[0], float):  # Not a step
            if last_event is not None:
                new_steps.append([beat_count] + last_event)
            last_event = None
            beat_count = 0
            new_steps.append(s)
        elif s[1:].count(0) != (len(s) - 1) or last_event is None:  # Non-empty
            if last_event is not None:
                new_steps.append([beat_count] + last_event)
            last_event = s[1:]
            beat_count = s[0]
...
class MirrorTransform(MappingTransform):
    def __init__(self, mode):
        MappingTransform.__init__(self, mode, 1)
...
class RightTransform(MappingTransform):
    def __init__(self, mode):
        MappingTransform.__init__(self, mode, 3)
    ...
    def _update_state(self, steps):
        if steps[0] not in NOT_STEPS:
            ...
            for i, s in enumerate(steps[1:]):
                if s & 1 and i in self._holds:
                    self._holds.remove(i)
                if s & 2:
                    self._holds.append(i)
...
def size(steps, opt):
    if opt == 1:
        ...
        little(steps, 2)  # Little
    elif opt == 3:
        insert_taps(steps, 4.0, 2.0, False)  # Big
    elif opt == 4:
        insert_taps(steps, 2.0, 1.0, False)  # Quick
    elif opt == 5:
        insert_taps(steps, 4.0, 3.0, True)  # Skippy
    ...
    for i, step in enumerate(steps[:-1]):
        if isinstance(step[0], float):  # This is a note...
            for j, s in enumerate(step[1:]):
                if s & 2:
                    holds.append(j)
                elif s & 1 and j in holds:
                    holds.remove(j)

            if not isinstance(steps[i + 1][0], float):
                new_steps.append(step)  # Next isn't a note.

            elif step[1:].count(0) == len(step[1:]) or steps[i + 1][1:].count(0) == len(
                steps[i + 1][1:]
            ):
            ...
            if int(beat + 0.00001) > int(beat):
                beat = int(beat + 0.00001)
        else:
            if steps[i][0] == "D":
                beat += steps[i][1]
            new_steps.append(steps[i])
    ...
    def _transform(self, s):
        if s[0] not in NOT_STEPS:
            ...
            for i, s in enumerate(step):
                if s & 2 and i not in self._holds:
                    self._holds.append(i)

            if step.count(0) < len(step) - 1:
                if self._side and not self._holds:
                    step.reverse()
...
class WideTransform(Transform):
    ...
    def _update_state(self, s):
        if s[0] not in NOT_STEPS:
            for i, si in enumerate(s[1:]):
                if si & 1 and i in self._holds:
                    self._holds.remove(i)

    def _transform(self, s):
        if s[0] not in NOT_STEPS:
            step = s[1:]
            if self._beat % 4 == 0 and s.count(0) == len(step) - 1 and len(self._holds) == 0:
                first = 0
                while step[first] == 0:
                    first += 1  # Find the first step
                to_add = int(math.sqrt(self._beat)) % len(step)
                if step[to_add] != 0:
                    to_add = (to_add + 1) % len(step)
                step[to_add] = 1
                s = [s[0]] + step
...
class PanelTransform(Transform):
    ...
    def _generate_accept(self):
        ...
        for dir in PanelTransform.accept:
            if dir in self.orig_panels:
                accept[dir] = "".join(
                    [c for c in PanelTransform.accept[dir] if c in self.new_panels]
                )
    ...
    def _generate_transform(self):
        ...
        for d in unmapped:
            accept = [a for a in self.accept[d] if trans[self.new_panels.index(a)] is None]
    ...
    def _update_state(self, steps):
        ...
        if not isinstance(steps[0], float):
            return
...
def generate_mode(song, difficulty, target_mode, pid):
    ...
    if target_mode in games.SINGLE:
        mode = "SINGLE"
    elif target_mode in games.VERSUS:
        mode = "VERSUS"
    elif target_mode in games.ONLY_COUPLE:
        mode = "COUPLE"
    elif target_mode in games.DOUBLE:
        mode = "DOUBLE"
    ...
    if mode in song.steps:
        ...
        if target_mode[:3] == "DMX":
            if target_mode in games.COUPLE:
                ...
                return song.steps[mode][difficulty]
        ...
        mode = equiv[mode]
        if len(games.GAMES[target_mode].dirs) == 4:
            T = FiveToFourTransform
        else:
            T = PanelTransform
    else:
        print("This shouldn't happen! Email pyddr-devel@icculus.org.")

    if mode in games.COUPLE:
        steps = steps[pid]
    ...
    trans = T(
        games.GAMES[mode].dirs,
        games.GAMES[target_mode].dirs,
