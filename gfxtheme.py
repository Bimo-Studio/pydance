...
class ThemeFile:
    ...
    def is_theme(cls, filename, game):
        if not os.path.isdir(filename):
            ...
            return False
        elif os.path.split(os.path.split(filename)[0])[1] != "%dx%d" % (
            game.width,
            game.width,
        ):
            return False
            ...
            for dir in game.dirs:
                ...
                for dirset in ThemeFile.rotate:
                    if dir in dirset:
                        for d in dirset:
                            possible = "arr_{}_{}_0.png".format("%s", d)
                            if os.path.exists(
                                os.path.join(filename, possible % "c")
                            ) and os.path.exists(os.path.join(filename, possible % "n")):
                                found = True
                if not found:
                    return False
        return True
    ...
    def get_image(self, image_name):
        try:
            if self.zip:
                return pygame.image.load(BytesIO(self.zip.read(image_name))).convert()
            else:
    ...
    def has_image(self, image_name):
        if self.zip:
            return image_name in self.zip.namelist()
        else:
    ...
    def get_arrow(self, type, dir, num):
        ...
        if not self.has_image(fn):
            for dirset in ["ldur", "kzwg", "c"]:
                if dir in dirset:
                    for d in dirset:
                        rotate = ThemeFile.rotate[dirset][dir] - ThemeFile.rotate[dirset][d]
...
class GFXTheme:
    def __init__(self, name, pid, game):
        ...
        if self.path is None:
            raise RuntimeError(f"E: Cannot load theme '{size}/{name}'.")
...
class Arrow:
    def __init__(self, theme, type, dir, color, left):
        ...
        if self._image.get_width() != theme.size or self._image.get_height() != theme.size:
...
class TopArrow(Listener, pygame.sprite.Sprite):
    def __init__(self, direction, ypos, pid, theme, game):
        ...
        self.dir = direction
        left = (
            game.left_off(pid) + game.player_offset * pid + game.dirs.index(direction) * game.width
        )
    ...
    def stepped(self, pid, dir, time, etime, rating, combo):
        if self.pid != pid or self.dir != dir or rating == "M":
            return
        self._pressed = True
        self.endpresstime = time + 0.25  # Number of seconds to change it for.

    def update(self, time, beat):
        if time > self.endpresstime:
            self._pressed = False
        if self._pressed:
            self.image = self.sarrow.get_image(beat)
        else:
            self.image = self.narrow.get_image(beat)
...
class ArrowFX(Listener, pygame.sprite.Sprite):
    def __init__(self, direction, ypos, pid, theme, game):
        ...
        self.centerx = (
            game.left_off(pid) + game.dirs.index(direction) * game.width + game.width // 2
        )
        self.pid = pid
        ...
        self.direction = 1
        self.holdtype = 0
        self.colors = {"V": [255, 255, 255], "P": [255, 255, 0], "G": [0, 255, 0]}
    ...
    def stepped(self, pid, dir, time, etime, tinttype, combo):
        if pid != self.pid or dir != self.dir:
            return
        elif tinttype not in self.colors:
            return
...
# Alias used by dance.py / local scripts expecting the old name
GfxTheme = GFXTheme
