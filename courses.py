...
class AbstractCourse:
    ...
    def _find_difficulty(self, song, diff):
        if self.gametype not in song.difficulty:
            return False
        ...
        # possible ratings to choose.
            ...
            if diff in song.difficulty[self.gametype]:
                return diff
            else:
                return False
    ...
    def next(self):
        ...
        if isinstance(diff, list):
            ...
            a, b = list(map(int, diff.split("..")))
        elif len(diff) < 3:
            a, b = int(diff), int(diff)
        if a or b:
            diff = list(range(a, b + 1))
        ...
        if name[0] == _("BEST"):
            ...
            if "/" in name:
                ...
                if folder in self.all_songs:
                    songs = [
                        s
                        for s in self.all_songs[folder].values()
                ...
                # Any random song.
                songs = []
                for v in self.all_songs.values():
                    songs.extend(list(v.values()))

            songs = [
                s
                for s in songs
                if (s, diff) not in self.past_songs and self._find_difficulty(s, diff)
            ]
    ...
    def __init__(self, filename, all_songs, recordkeys):
        ...
        for line in lines:
            if line[0] == "COURSE":
                ...
                if name[0:4] == _("BEST"):
                    name = (_("BEST"), int(name[4:]))
                elif name[0:5] == _("WORST"):
                    name = (_("WORST"), int(name[5:]))
                else:
...
def make_players(all_songs, recordkeys):
    ...
    for start, end in [(1, 5), (5, 9), (1, 9)]:
        for diff_name, diffs in [
            (_("(Easy)"), ["BASIC", "LIGHT", "EASY"]),
            (_("(Normal)"), ["ANOTHER", "STANDARD", "NORMAL", "TRICK", "MEDIUM"]),
            (_("(Hard)"), ["MANIAC", "HEAVY", "HARD"]),
            (_("(Expert)"), ["HARDCORE", "CRAZY", "SMANIAC", "EXPERT"]),
        ]:
            ...
            for name, type in (
                ...
                courses.append(
                    CodedCourse(all_songs, recordkeys, realname, _("Player's Picks"), songs)
                )
    ...
    for i in [4, 8, 12]:
        ...
        randsongs = [("*", "0..10", {})] * i
        courses.append(
            CodedCourse(all_songs, recordkeys, randname, _("random.choose(songs)"), randsongs)
        )
    ...
    courses.append(
        CodedCourse(all_songs, recordkeys, _("Primal Fear (Easy)"), _("Player's Picks"), songs1)
    )
    courses.append(
        CodedCourse(all_songs, recordkeys, _("Primal Fear (Hard)"), _("Player's Picks"), songs2)
    )
    return courses
