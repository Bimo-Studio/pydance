...
try:
    records = pickle.load(open(record_fn, "rb"))
except Exception:
    records = {}
bad_records = {}
...
def best(index, diffs, game):
    ...
    if not isinstance(diffs, list):
        diffs = [diffs]
    index -= 1
    r = [(v[0], k[0]) for k, v in records.items() if (k[1] in diffs and k[2] == game)]
    if len(r) == 0:
        return None
    r.sort()
    r.reverse()
...
def like(index, diffs, game):
    ...
    if not isinstance(diffs, list):
        diffs = [diffs]
    index -= 1
    r = [(v[2], k[0]) for k, v in records.items() if (k[1] in diffs and k[2] == game)]
    if len(r) == 0:
        return None
    r.sort()
    r.reverse()
