...
class BPMDisplay(pygame.sprite.Sprite):
    ...
    def update(self, time):
        ...
        if len(self._bpms) == 0:
            ...
            t -= 2000
            p = t / 1000.0
            self._bpm = p * self._bpms[self._bpm_idx] + (1 - p) * self._bpms[self._bpm_idx - 1]
...
class ScrollingImage(pygame.sprite.Sprite):
    ...
    def set_image(self, image):
        if image.get_height() > self._height:
            ...
            self.image = pygame.Surface([image.get_width(), self._height], SRCALPHA, 32)
            self._image = image
...
class HelpText(pygame.sprite.Sprite):
    def __init__(self, strs, color, bgcolor, font, center):
        pygame.sprite.Sprite.__init__(self)
        self._idx = -1
        self._strings = [(s, font.render(s, True, color, bgcolor).convert()) for s in strs]
        self._center = center
...
class ActiveIndicator(pygame.sprite.Sprite):
    def __init__(self, topleft, width=220, height=26):
        ...
        bar.blit(left, [0, 0])
        bar.blit(pygame.transform.scale(mid, [width - 10, bar.get_height()]), [5, 0])
...
class ListBox(pygame.sprite.Sprite):
    ...
    def item_index_at_screen_pos(self, mx, my):
        """If (mx, my) hits this list, return item index 0..len-1; else None."""
        if not self._items or not self.rect.collidepoint(mx, my):
            return None
        local_y = my - self.rect.top
        row = min(max(0, local_y // self._h), self._count - 1)
        n = len(self._items)
        return (self._idx + row) % n
...
class BannerDisplay(pygame.sprite.Sprite):
    ...
    def _render(self):
        ...
        if self._subtitle:
            self.image.blit(self._subtitle, self._r_s)
...
class WrapTextDisplay(pygame.sprite.Sprite):
    def __init__(self, font, width, topleft, str=" ", centered=False, color=None):
        pygame.sprite.Sprite.__init__(self)
        self._text = str
        ...
        self._topleft = topleft
        self._color = color if color is not None else MENU_NAV_TEXT
        self._render()

    def _render(self):
        self.image = self._font.render(
            self._text, shdw=False, centered=self._centered, color=self._color
        )
        self.rect = self.image.get_rect()
        self.rect.topleft = self._topleft
...
class InterfaceWindow:
    def __init__(self, screen, bg_fn):
        self._screen = screen
        _ = bg_fn  # unused; menu backgrounds are plain white (no PNG)
        self._bg = pygame.Surface(screen.get_size())
        self._bg.fill((255, 255, 255))
