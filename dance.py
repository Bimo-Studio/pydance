...
#
# New work should go into dance2.py, which pydance.py imports as the active
# implementation (import dance2 as dance). This file remains for reference
...
class SongInfoScreen(InterfaceWindow):
    def __init__(self, song, diffs, playmode, songconf, configs, screen):
        ...
        self._countdown = WrapTextDisplay(
            FontTheme.SongInfo_countdown, 68, [285, 429], centered=True
        )
        ...
        self._other_widgets = [
            TextDisplay("SongInfo_screen_title", [400, 50], [15, 35], _("Next Song")),
            TextDisplay(
                "SongInfo_controls",
                [259, 37],
                [15, 450],
                _("Confirm: Begin   Cancel: Stop"),
            ),
            TextDisplay(
                "SongInfo_controls",
                [259, 37],
                [380, 450],
                _("F1 / Start: Change Options"),
            ),
        ]

        self._sprites.add(
            [self._banner, self._countdown]
            + self._player_widgets
            + self._player_opts
            + self._other_widgets
        )
    ...
    def player_widgets(self, pid):
        v_top = 11 + pid * 210
        pname = WrapTextDisplay(
            FontTheme.SongInfo_player,
            248,
            [380, v_top],
            _("Player %d") % (pid + 1,),
            centered=True,
        ...
        d.set(
            self.diffs[pid],
            DIFF_COLORS.get(self.diffs[pid], [127, 127, 127]),
    ...
    def player_opts(self, pid):
        ...
        return [
            WrapTextDisplay(
                FontTheme.SongInfo_current_options,
                248,
                [380, v_top + 95],
                self.opt_summary(pid),
                centered=True,
            )
        ]
    ...
    def opt_summary(self, pid):
        ...
        for opt, special_display in changeable_between:
            if self.configs[pid][opt] == default_conf[opt]:
                continue
...
def play(screen, playlist, configs, songconf, playmode):
    ...
    for songfn, diff in playlist:
        try:
            ...
            error.ErrorMessage(screen, _("There was an error loading ") + os.path.split(songfn)[1])
            first = True
            continue
        ...
        if display_songinfo:
            if not SongInfoScreen(
                current_song, diff, playmode, songconf, configs, screen
            ).proceed_to_song:
                break
            for playerID in range(numplayers):
                for opt, dummy in changeable_between:
                    setattr(players[playerID], opt, configs[playerID][opt])

        for pid, player in enumerate(players):
            player.set_song(current_song, diff[pid], songdata.lyricdisplay)

        print(_("Playing"), songfn)
        print(songdata.title, "by", end=" ")
    ...
    if songs_played == 1 and not players[0].escaped:
        for p in players:
            if not p.failed:
                ...
                records.add(current_song.info["recordkey"], diff[p.pid], playmode, -2, " ")
...
def dance(screen, song, players, prevscr, ready_go, game):
    ...
    if song.crapout != 0:
        error.ErrorMessage(
            screen,
            _("The audio file for this song ") + song.filename + _(" could not be found."),
        )
    ...
    while True:
        ...
        for plr in players:
            plr.get_next_events(song, curtime)
        ...
        if backmovie is None:
            # pygame.display.update([]) refreshes nothing; match dance2 fallback.
            if rectlist:
        ...
        if (curtime > players[0].length - 1) and (songtext.zdir == 0) and (songtext.zoom > 0):
            songtext.zout()
            grptext.zout()

    if fpstext:
        print(_("Average FPS for this song was %d.") % fpstext.fps())
    return songFailed


if __name__ == "__main__":
    # Legacy file: active gameplay is dance2; pydance.main() runs the menu and game.
    import pydance
