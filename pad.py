...
A6B12 = {1: UPRIGHT, 3: UPLEFT, 8: SELECT, 4: LEFT, 5: RIGHT, 6: DOWN, 7: UP, 9: START}
...
class Pad:
    ...
    def __init__(self, handler=pygame.event):
        ...
        for i in range(totaljoy):
            ...
            if args[2] == 32:
                emsusb2 = i
            elif mat is None and (args[1], args[2]) in MATS:
                mat = i
            elif mat2 is None and (args[1], args[2]) in MATS:
                mat2 = i
        ...
        if loaded_input:
            ...
            if mat2:
                ...
                axes, but = (
                    joy.get_numaxes() + 2 * joy.get_numhats(),
                    joy.get_numbuttons(),
                )
    ...
    def device_key_for(self, keyboard, pid, event):
        for (device, key), (p, e) in self.events.items():
            if p == pid and e == event:
                if keyboard and device == -1:
                    return pygame.key.name(key)
                elif not keyboard and device != -1:
                    return "%d:%d" % (device, key)
        return "---"

    def delete_event(self, pid, keyb, event):
        # Snapshot keys: deleting during .items() iteration raises RuntimeError.
        to_remove = []
        for (d, k), (p, e) in list(self.events.items()):
            if p == pid and e == event and ((d == -1 and keyb) or (d != -1 and not keyb)):
                to_remove.append((d, k, p, e))
    ...
    def delete_events(self, pid):
        for k, v in list(self.events.items()):
            if v[0] == pid:
                del self.events[k]
        for pe in list(self.states.keys()):
            if pe[0] == pid:
                del self.states[pe]
    ...
    def poll(self):
        ...
        if (ev.type == KEYDOWN or ev.type == KEYUP) and (
            ev.key == K_LEFT or ev.key == K_RIGHT or ev.key == K_UP or ev.key == K_DOWN
        ):
            default = (
                0,
                {K_LEFT: LEFT, K_RIGHT: RIGHT, K_UP: UP, K_DOWN: DOWN}[ev.key],
            )
...
class PadConfig:
    ...
    def __init__(self, screen):
        ...
        while ev.type != KEYDOWN or ev.key != K_ESCAPE:
            ...
            if ev.type == KEYDOWN:
                if ev.key in [K_LEFT, K_KP4]:
                    self.loc[0] -= 1
                elif ev.key in [K_RIGHT, K_KP6]:
                    self.loc[0] += 1
                elif ev.key in [K_UP, K_KP8]:
                    self.loc[1] -= 1
                elif ev.key in [K_DOWN, K_KP2]:
                    self.loc[1] += 1
    ...
    def map_key(self):
        ...
        while ev.type != wanted_type and (ev.type != KEYDOWN or ev.key != K_ESCAPE):
            ev = pygame.event.wait()
        if ev.type != KEYDOWN or ev.key != K_ESCAPE:
            if keyb:
                dev, but = -1, ev.key
            else:
                dev, but = ev.joy, ev.button
    ...
    def render(self):
        ...
        for t in text:
            ...
            for t2 in text2:
                ...
                r = img.get_rect()
                r.midtop = [
                    cent + offset * text2.index(t2) + (offset * 3) * text.index(t),
                    50,
                ]
                self.screen.blit(img, r)
        ...
        for dir in PadConfig.directions:
            ...
            for i, o in enumerate(order):
                if dir == self.loc[1] + 2 and (
                    (i < 2 and i == self.loc[0]) or (i > 2 and i == self.loc[0] + 1)
                ):
                    img = FontTheme.MapKeys_entries.render(_(o), True, colors.WHITE)
                else:
                    img = FontTheme.MapKeys_entries.render(_(o), True, colors.BLACK)
                r = img.get_rect()
