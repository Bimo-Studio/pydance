...
class Announcer(Listener):
    ...
    def __init__(self, name):
        ...
        with open(os.path.join(filename, "djtheme.cfg")) as fi:
            for line in fi:
    ...
    def say(self, sec, mood=(0, 100)):
        if sec not in self.sections or len(self.sections[sec]) == 0:
            return
        n = len(self.sections[sec])
        # Normalize mood wrt the number of choices
        try:
            mood = (int(mood[0] / 100.0 * n), min(int(mood[1] / 100.0 * n), n - 1))
        except TypeError:
            # Use a random variance of 10% if a scalar is given
            mood = (
                int(max(mood / 100.0 - 0.1, 0) * n),  # don't < 0
                int(min(mood / 100.0 + 0.1, 0.999) * n),
            )  # don't == n
