...
class Credits(pygame.sprite.Sprite):
    def __init__(self, lines):
        ...
        self._w = max([self._font.size(line)[0] for line in self._lines])
        self._h = max([self._font.size(line)[1] for line in self._lines])
        self.update()
...
def switch_tuple(name, list):
    ...
    for item in list:
        if next:
            ...
            next = True
    if next:
        mainconfig[name] = list[0][0]
    elif mainconfig[name] == orig:
        mainconfig[name] = list[0][0]
    return get_tuple(name, list)


def switch_tuple_back(name, items):
    items = items[:]
    items.reverse()
    return switch_tuple(name, items)
...
def do(screen, songdata):
    ...
    rotate_index_opt = {
        ui.OPTIONS: switch_rotate_index,
    ...
    try:
        with open(os.path.join(pydance_path, "CREDITS")) as _cf:
            lines = _cf.read().split("\n")
        lines = list(lines)
    ...
    m = (
        [
            _("Play Game"),
            {ui.OPTIONS: wrap_ctr, ui.CONFIRM: wrap_ctr},
            (GameSelect, songdata),
        ],
        [
            _("Map Keys"),
        ...
        (
            ...
            (
                ...
                [_("DMX"), rotate_opt, ("dmx-theme", ThemeFile.list_themes("DMX"))],
                [
                    _("EZ2"),
                    rotate_opt,
        ...
        (
            ...
            [
                ...
                (
                    "explodestyle",
                    (_("none"), _("rotate"), _("scale"), _("rotate & scale")),
                ),
            ],
        ...
        (
            ...
            [
                _("Song Previews"),
                tuple_opt,
                ("previewmusic", [(0, "Off"), (1, "On"), (2, "Safe")]),
            ],
            ...
            [
                ...
                (
                    "songinfoscreen",
                    list(zip([0, 1, 2], [_("Never"), _("Multi-song Only"), _("Always")])),
                ),
            ],
