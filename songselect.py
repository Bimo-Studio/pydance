...
SORTS: dict[str, Callable[[Any], Any]] = {}
SORTS["subtitle"] = lambda x: x.info["subtitle"].lower()
SORTS["title"] = lambda x: (x.info["title"].lower(), SORTS["subtitle"](x))
SORTS["artist"] = lambda x: (x.info["artist"].lower(), SORTS["title"](x))
SORTS["bpm"] = lambda x: (x.info["bpm"], SORTS["title"](x))
SORTS["mix"] = lambda x: (x.info["mix"], SORTS["title"](x))
SORTS["rating"] = lambda x: (x.difficulty[x.diff_list[0]], SORTS["rank"](x))
SORTS["difficulty"] = lambda x: (
    util.difficulty_sort_key(x.diff_list[0]),
    SORTS["rating"](x),
)
SORTS["rank"] = lambda x: -records.get(x.info["recordkey"], x.diff_list[0], game)[0]
...
class FolderDisplay:
    ...
    def render(self):
        ...
        if self.banner is None:
            self.banner = SongItemDisplay.no_banner
...
class SongPreview:
    ...
    def preview(self, song):
        if mainconfig["previewmusic"] and not song.isfolder:
            if not song.info.get("filename"):
                return
            if song.info["filename"].lower().endswith("mp3") and mainconfig["previewmusic"] == 2:
                music.stop()
                self._playing = False
                return
    ...
    def update(self, time):
        if self._filename is None:
            ...
            try:
                ...
                music.play(0, 0)
                self._playing = True
            except Exception:  # Filename not found? Song is too short? SMPEG blows?
                music.stop()
                self.playing = False
...
class SongSelect(InterfaceWindow):
    def __init__(self, songs, courses, screen, game):
        ...
        if len(songs) == 0:
            error.ErrorMessage(
                screen,
                _("You don't have any songs for the game mode (")
                + game
                + _(") that you selected."),
        ...
        self._list = ListBox(FontTheme.SongSel_list, MENU_NAV_TEXT, 26, 16, 220, [408, 56])
        ...
        for i in range(players):
            ...
            if not self._song.isfolder:
                ...
                rank = records.get(self._song.info["recordkey"], diff_name, self._game)[0]
                grade = grades.grades[self._config["grade"]].grade_by_rank(rank)
                d.set(
                    diff_name,
                    DIFF_COLORS.get(diff_name, [127, 127, 127]),
                    self._song.difficulty[diff_name],
                    grade,
                )
            else:
    ...
    def loop(self):
        pid, ev = ui.ui.poll()
        nav_ud = ui.MenuListNavDedupe()
        self._list.set_index(self._index)
        self._title.set_text(self._base_text + " - %d/%d" % (self._index + 1, len(self._songitems)))
        while not (ev == ui.CANCEL and (not self._folders or self._song.isfolder)):
            ...
            if pid >= len(self._diff_names):
                ...
                ui.LEFT,
                ui.RIGHT,
            ):  # Need LEFT/RIGHT player-specific
                ev = ui.PASS

            list_changed_by_mouse = False
            if ev == ui.MOUSE_CLICK:
                mx, my = ui.ui.last_mouse_pos
                hit = self._list.item_index_at_screen_pos(mx, my)
                if hit is not None:
                    if hit == self._index:
                        ev = ui.CONFIRM
                        ui.ui.clear()
                        ...
                        self._index = hit
                        list_changed_by_mouse = True
                        ev = ui.PASS

            list_ud = ev != ui.PASS and ev in (ui.UP, ui.DOWN) and nav_ud.consume_ud(pid, ev)
            if list_ud:
                if ev == ui.UP:
                    self._index -= 1
                else:
            ...
            if ev in [ui.CANCEL, ui.UP, ui.DOWN, ui.RANDOM, ui.CONFIRM, ui.SORT] or list_changed_by_mouse:
                if ev == ui.UP:
                    ...
                    self._list.set_index(self._index, 0)  # don't animate
                self._title.set_text(
                    self._base_text + " - %d/%d" % (self._index + 1, len(self._songitems))
                )

            if ev in [ui.UP, ui.DOWN, ui.RANDOM, ui.SORT, ui.CONFIRM] or list_changed_by_mouse:
                if not self._song.isfolder:
                    for pl, dname in enumerate(self._diff_names):
                        ...
                        if name in self._song.diff_list:
                            ...
                            if (
                                name in util.DIFFICULTY_LIST
                                or self._unify_difficulties(name) in util.DIFFICULTY_LIST
                            ) and reduce(
                                lambda a, b: a and b in util.DIFFICULTY_LIST,
                                self._song.diff_list,
                                True,
                            ):
                                name = self._unify_difficulties(name)
                                namei = util.DIFFICULTY_LIST.index(name)
                                diffi = [
                                    util.DIFFICULTY_LIST.index(d) for d in self._song.diff_list
                                ]
                                dds = [abs(d - namei) for d in diffi]
                                self._diff_names[pl] = self._song.diff_list[dds.index(min(dds))]
    ...
    def _unify_difficulties(self, difficulty):
        ...
        if diff not in util.DIFFICULTY_LIST:
            diffs = [d for d in util.DIFFICULTY_LIST if d.startswith(diff[:2])]
            if len(diffs) > 0:
                diff = diffs[0]
            else:
                diff = "other"
        if diff == "S-MANIAC":
            diff = "SMANIAC"  # see constants.DIFF_COLORS
        return diff
    ...
    def _create_folder_list(self):
        ...
        new_songs = [
            FolderDisplay(folder, sort_name, len(self._folders[sort_name][folder]))
            for folder in lst
        ]
    ...
    def _create_song_list(self, folder):
        ...
        self._list.set_items([s.info["title"] for s in self._songitems])
        if self._folders:
            self._base_text = folder_name(folder, sort_name)
    ...
    def _find_resorted(self):
        if (
            isinstance(self._song, SongItemDisplay)
            and SORT_DANCES[SORT_NAMES[mainconfig["sortmode"] % NUM_SORTS]]
        ):
            diff = self._diff_names[self._last_player]
            return self._song.danceitems[diff]
        elif (
            isinstance(self._song, DanceItemDisplay)
            and not SORT_DANCES[SORT_NAMES[mainconfig["sortmode"] % NUM_SORTS]]
        ):
