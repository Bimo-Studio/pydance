...
class DancePointsGrade(AbstractGrade):
    ...
    def grade(self, failed):
        if failed:
            ...
            return DancePointsGrade.grade_by_rank(self.rank())
...
class EndlessGrade(DancePointsGrade):
    def grade(self, failed):
        return DancePointsGrade.grade(self, False)


grades = [DancePointsGrade, EndlessGrade]
grade_opt = [
    (0, _("Dance Points"), ""),
    (1, _("Endless"), _("Like dance points, but failing doesn't result in an F.")),
]
