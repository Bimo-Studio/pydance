...
class AbstractArrow(pygame.sprite.Sprite):
    ...
    samples = {}
    for d in ["u", "d", "l", "r"]:
        samples[d] = pygame.mixer.Sound(os.path.join(sound_path, "assist-%s.ogg" % d))
    ...
    def __init__(self, arrow, beat, secret, player, song):
        ...
        if self.battle:
            ...
            self.centerx = self.rect.centerx = self.goalcenterx
    ...
    def set_alpha(self, curtime, beatsleft, top, factor):
        ...
        # NB - Making a new surface, then blitting the image in place, == 20%
    ...
    def scale_spin_battle(self, image, top, pct):
        ...
        if self.battle:
            if pct > 4.5 / 6:
                rect.centerx = self.origcenterx
            elif pct > 2.0 / 6:
                p = (pct - 2.0 / 6) / (2.5 / 6)
                rect.centerx = (1 - p) * self.goalcenterx + p * self.origcenterx
            else:
                rect.centerx = self.goalcenterx
        else:
...
class HoldArrowSprite(AbstractArrow):
    def __init__(self, arrow, beats, secret, times, player, song):
        ...
        if self.timef2 is None:
            self.timef2 = self.timef1  # ?
    ...
    def broken_at(self, time, judge):
        if self._broken_at == -1:
            self._broken_at = time
        elif time - self._broken_at > judge.ok_time:
            self.broken = True
        return self.broken
    ...
    def update(self, curtime, curbpm, beat, judge):
        ...
        if self.target_bpm is None:
            ...
            if self.accel == 1:
                ...
                speed_top = speed_bottom = self.speed
            ...
            # See the notes in ArrowSprite about the derivation of this.
            if self.bottom > self.top:
                top = self.top + self.vector * int(
                    p_top * timeleft_top * self.target_bpm / 60.0 * 64
                )
                bottom = self.top + self.vector * int(
                    p_bottom * timeleft_bot * self.target_bpm / 60.0 * 64
                )
            else:
                top = self.top + self.vector * int(
                    p_top * timeleft_bot * self.target_bpm / 60.0 * 64
                )
                bottom = self.top + self.vector * int(
                    p_bottom * timeleft_top * self.target_bpm / 60.0 * 64
                )
