...
class GameType:
    ...
    def __init__(self, **args):
        ...
        if self.double:
            self.player_offset = 640 // (2 * self.players)
        else:
        ...
        if self.double:
            self.left_offset = (640 // (2 * self.players) - self.width * len(self.dirs)) // 2
        else:
        ...
        for d in self.dirs:
            self.battle_lefts[d] = int(self.width * (len(self.dirs) / 2.0 - self.dirs.index(d)))
    ...
    def left_off(self, pid):
        if not self.double and not self.centered:
            return self.left_offset
        elif pid & 1:
            return 0
        else:
            return self.left_offset * 2
...
GAMES = {
    ...
    "5COUPLE": GameType(players=2, couple=True, dirs="wkczg", width=56, theme="5p"),
    "5DOUBLE": GameType(players=1, double=True, dirs="wkczg", width=56, theme="5p"),
    "6PANEL": GameType(players=1, dirs="lkduzr", theme="6pl"),
    "6VERSUS": GameType(players=2, dirs="lkduzr", width=48, theme="6ps"),
    "6COUPLE": GameType(players=2, couple=True, dirs="lkduzr", width=48, theme="6ps"),
    "6DOUBLE": GameType(players=1, double=True, dirs="lkduzr", width=48, theme="6ps"),
    "8PANEL": GameType(players=1, dirs="wlkduzrg", theme="8pl"),
    "8VERSUS": GameType(players=2, dirs="wlkduzrg", width=32, theme="8ps"),
    "8COUPLE": GameType(players=2, dirs="wlkduzrg", width=32, couple=True, theme="8ps"),
    "8DOUBLE": GameType(players=1, dirs="wlkduzrg", width=32, double=True, theme="8ps"),
    "9PANEL": GameType(players=1, dirs="wlkdcuzrg", theme="9pl"),
    "9VERSUS": GameType(players=2, dirs="wlkdcuzrg", width=32, theme="9ps"),
    "9COUPLE": GameType(players=2, dirs="wlkdcuzrg", width=32, couple=True, theme="9ps"),
    "9DOUBLE": GameType(players=1, dirs="wlkdcuzrg", width=32, double=True, theme="9ps"),
    "EZ2SINGLE": GameType(
        players=1,
        dirs="kldrz",
        width=56,
    ...
    "EZ2REAL": GameType(players=1, dirs="klwdgrz", width=56, theme="ez2real", theme_default="ez2"),
    "REALVERSUS": GameType(
        players=2, dirs="klwdgrz", width=32, theme="ez2real", theme_default="ez2"
    ),
    "REALCOUPLE": GameType(
        players=2,
        dirs="klwdgrz",
    ...
    "PARAPARA": GameType(players=1, dirs="lkuzr", width=48, theme="para"),
    "PARAVERSUS": GameType(players=2, dirs="lkuzr", width=48, theme="para"),
    "PARACOUPLE": GameType(players=2, couple=True, dirs="lkuzr", width=48, theme="para"),
    "PARADOUBLE": GameType(players=1, double=True, dirs="lkuzr", width=48, theme="para"),
    "DMX": GameType(players=1, dirs="lkzr", width=32, theme="dmx", theme_default="dmxesque"),
    "DMXVERSUS": GameType(
        players=2,
        dirs="lkzr",
        width=32,
...
SINGLE = [mode for mode in GAMES if (GAMES[mode].players == 1 and not GAMES[mode].double)]
VERSUS = [mode for mode in GAMES if (GAMES[mode].players == 2 and not GAMES[mode].couple)]
COUPLE = [mode for mode in GAMES if GAMES[mode].couple]
ONLY_COUPLE = [mode for mode in GAMES if (GAMES[mode].couple and not GAMES[mode].double)]
