...
class SongEvent:
    ...
    def __repr__(self):
        rest = []
        if self.feet:
            rest.append("feet=%r" % self.feet)
        if self.extra:
            rest.append("extra=%r" % self.extra)
        if self.extra:
            rest.append("color=%r" % self.color)
        if self.appear:
            rest.append("appear=%r" % self.appear)
...
class Steps:
    def __init__(self, song, difficulty, player, pid, lyrics, playmode, offset):
        ...
        if playmode in song.steps:
            ...
            if playmode in games.COUPLE:
                song_steps = song_steps[pid]
        ...
        if player.size:
            stepfilters.size(song_steps, player.size)
        if player.jumps != 1:
            ...
            if words[0] == "W":
                ...
                if arrowcount != 0:
                    ...
                    if self.target_bpm is None:
                        ...
                        while (
                            time_to_add - time_led > 0
                            and beat_led
                            > (time_to_add - time_led - self.lastbpmchangetime[bpm_i][0])
                            * self.lastbpmchangetime[bpm_i][1]
                            / 60
                        ):
                            beat_led -= (
                                (time_to_add - time_led - self.lastbpmchangetime[bpm_i][0])
        ...
        step_trace.log_steps_loaded(self, getattr(song, "filename", "?"))
    ...
    def get_events(self, curtime):
        """curtime must match the game loop (same source as Player.game_loop)."""
        events, nevents = [], []
        idx = self.event_idx
        nidx = self.nevent_idx
        time = self.curtime = float(curtime)
        while idx < len(self.events) and self.events[idx].when <= time + 2 * toRealTime(
            self.events[idx].bpm, 1
        ):
        ...
        # Skip events with no ``appear`` (e.g. the initial BPM/extra marker at index 0).
        # Otherwise ``nevent_idx`` stays 0 forever: ``appear is None`` short-circuits the
        # old ``and`` chain so we never advance, and no scrolling arrows spawn.
        while nidx < len(self.events):
            ev = self.events[nidx]
            if ev.appear is None:
                nidx += 1
                continue
            if ev.appear <= time:
                self.playingbpm = ev.bpm
                nevents.append(ev)
                nidx += 1
            else:
                break
        ...
        step_trace.log_get_events(time, self, nevents)
...
class SongData:
    def __init__(self, song, config):
        if song.info["background"]:
            self.background = song.info["background"]
        else:
            self.background = os.path.join(image_path, "bg.png")
        ...
        for key in [
            ...
            setattr(self, key, song.info[key])

        self.soffset = song.info["gap"] * 1000

        self.crapout = 0

        # Fixed for Python 3 - attributes set individually
