...
# key= functions (Python 3; sort() no longer accepts cmp= callbacks)
SORTS = {
    "title": lambda x: x.name.lower(),
    "mix": lambda x: (str(x.mixname).lower(), x.name.lower()),
}
...
class CourseDisplay:
    ...
    def generate_songlist(self, recordkeys, game):
        ...
        for name, diff, mods in self.course.songs:
            if "*" in name:
                ...
                if song:
                    subtitle = (song.info["subtitle"] or "") + (_(" (Dislikes #%d)") % name[1])
                    name = song.info["title"]
                else:
    ...
    def render(self):
        ...
        for name, subtitle, diff in self._songs:
            ...
            if subtitle:
                t3 = small_font.render(subtitle, color=[190, 190, 190], indent="    ")
            ...
            if self._type == "mix":
                for dir in mainconfig["coursedir"].split(os.pathsep):
                    ...
                    if os.path.exists(fn):
                        self.banner, self.clip = load_banner(fn, False)
...
class CourseSelector(InterfaceWindow):
    def __init__(self, songs, courses, screen, game):
        ...
        recordkeys = {k.info["recordkey"]: k for k in songs}
        ...
        self._list = ListBox(FontTheme.Crs_list, MENU_NAV_TEXT, 32, 10, 256, [373, 150])
        if mainconfig["folders"]:
            ...
            self._courses.sort(key=SORTS[SORT_NAMES[mainconfig["sortmode"] % NUM_SORTS]])
        ...
        self._sprites.add(
            [self._list, self._list_gfx, self._title, self._coursetitle, self._banner]
        )
    ...
    def loop(self):
        ...
        self._title.set_text(self._base_text + " - %d/%d" % (self._index + 1, len(self._courses)))
        while not (ev == ui.CANCEL and (not self._folders or self._course.isfolder)):
            ...
            if list_ud:
                ...
                if self._course.isfolder:
                    ...
                    root_idx = [fol.name for fol in self._courses].index(
                        self._course.folder[SORT_NAMES[mainconfig["sortmode"]]]
                    )
                ...
                if self._folders:
                    ...
                    self._courses.sort(key=SORTS[SORT_NAMES[mainconfig["sortmode"]]])
                    self._index = self._courses.index(s)
                    self._list.set_items([s.name for s in self._courses])
                ...
                if self._course.isfolder:
                    ...
                    dance.play(self._screen, course, self._configs, self._config, self._game)
                    course.done()
            ...
            if (
                ...
                self._title.set_text(
                    self._base_text + " - %d/%d" % (self._index + 1, len(self._courses))
                )
            self.update()
            pid, ev = ui.ui.poll()
    ...
    def _create_folder_list(self):
        ...
        new_courses = [
            FolderDisplay(folder, sort, len(self._folders[sort][folder])) for folder in lst
        ]
    ...
    def _create_course_list(self, folder):
        ...
        courselist.sort(key=SORTS[sort])
