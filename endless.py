...
class EndlessDiffDisplay(pygame.sprite.Sprite):
    def __init__(self, pid, constraint):
        ...
        self._c = constraint
        self._ptext = fontfx.shadow(
            _("Player %d") % (pid + 1), FontTheme.Endless_player, colors.WHITE
        )
    ...
    def update(self, time):
        if self._oldval != (self._c.kind, self._c.value):
            if self._c.kind == "name":
                ctext = fontfx.shadow(
                    _("Select by Difficulty"), FontTheme.Endless_filter_by, colors.WHITE
                )
                i18ndifficulty = _(self._c.value)
                vtext = fontfx.shadow(
                    i18ndifficulty.capitalize(),
                    FontTheme.Endless_filter_range,
                    colors.WHITE,
                )
            elif self._c.kind == "number":
                ctext = fontfx.shadow(
                    _("Select by Rating"), FontTheme.Endless_filter_by, colors.WHITE
                )
                vtext = fontfx.shadow(
                    _("Between %d and %d") % self._c.value,
                    FontTheme.Endless_filter_range,
                    colors.WHITE,
                )
...
class Constraint:
    ...
    def meets(self, diffs):
        if self.kind == "name":
            if self.value in diffs:
                return True
            else:
                return False
        elif self.kind == "number":
            for k in diffs:
                if diffs[k] in range(self.value[0], self.value[1] + 1):
                    return True
            return False
...
class FakePlaylist:
    def __init__(self, songs, constraints, screen, mode):
        self.songs = [
            s
            for s in songs
            if (s.info["valid"] and check_constraints(constraints, s.difficulty[mode]))
        ]
        self.working = []
        self.mode = mode
...
class Endless(InterfaceWindow):
    def __init__(self, songitems, courses, screen, gametype):
        ...
        for song in songitems:
            if gametype in song.difficulty:
                for d in song.difficulty[gametype]:
                    if d in diff_count and d not in diffs:
                        diffs.append(d)
                    else:
                        diff_count[d] = True
        ...
        if games.GAMES[gametype].players == 2:
            if games.GAMES[gametype].couple:
        ...
        self._sprites.add(
            HelpText(ENDLESS_HELP, MENU_NAV_TEXT, [255, 255, 255], FontTheme.help, [320, 20])
        )
        ...
        while ev != ui.CANCEL:
            ...
            if ev == ui.CONFIRM:  # not elif !!!
                dance.play(
                    screen,
                    FakePlaylist(songitems, self.constraints, screen, gametype),
                    self.player_configs,
                    self.game_config,
                    gametype,
                )
            ...
            # Ignore unknown events
                ...
                if self.constraints[pid].kind == "name":
                    newi = min(len(diffs) - 1, diffs.index(self.constraints[pid].value) + 1)
                    self.constraints[pid].value = diffs[newi]
                elif self.constraints[pid].kind == "number":
