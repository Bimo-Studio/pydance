...
class AbstractScore(Listener, pygame.sprite.Sprite):
    ...
    def _set_text(self, text):
        ...
        txt = fontfx.embfade(
            text, FontTheme.Dance_score_display, 2, (tx, 24), colors.color[_("gray")]
        )
    ...
    def update(self, curtime):
        if self.score != self.oldscore:
            ...
            scoretext = FontTheme.Dance_score_display.render(
                str(int(self.score)), 1, (192, 192, 192)
            )
...
class PydanceScore(AbstractScore):
    def set_song(self, pid, bpm, text, count, hold, feet):
        ...
        if count == 0:
            count = 1  # Don't crash on empty songs.
...
class FirstScore(AbstractScore):
    ...
    def stepped(self, pid, dir, cur_time, etime, rating, combo):
        if rating is None:
            return
        if self.combo == combo == 0:
            return  # No points when combo == 0.

        self.combo += 1

        if combo == 0:  # The rating can't be great or better, since combo == 0.
            if rating == "O":
                self.score += self.combo / 4 * 100
            self.combo = 0
        else:
            if rating == "G":
...
class ExtremeScore(ThirdScore):
    def set_song(self, pid, bpm, text, count, hold, feet):
        ...
        self.inc = {"V": 10 * score_coeff, "P": 10 * score_coeff, "G": 5 * score_coeff}
...
class ExtremeNonstopScore(ThirdScore):
    ...
    def set_song(self, pid, bpm, text, count, hold, feet):
        ...
        self.inc = {"V": 10 * score_coeff, "P": 9 * score_coeff, "G": 5 * score_coeff}
