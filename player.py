...
from pygame.sprite import RenderClear, RenderUpdates
...
class OrderedRenderUpdates(RenderClear):
    ...
    def add(self, sprite):
        has = self.spritedict.__contains__
    ...
    def draw(self, surface):
        ...
        for s in spritelist:
            ...
            # Pygame 2 may use an empty Rect instead of int 0 for "never drawn".
            if not r:
...
class HoldJudgeDisp(Listener, pygame.sprite.Sprite):
    def __init__(self, pid, player, game):
        ...
        self.okimg = fontfx.shadefade(
            _("OK"), FontTheme.Dance_hold_judgment, 3, [48, 24], [112, 224, 112]
        )
        self.ngimg = fontfx.shadefade(
            _("NG"), FontTheme.Dance_hold_judgment, 3, [48, 24], [224, 112, 112]
        )
...
class JudgingDisp(Listener, pygame.sprite.Sprite):
    def __init__(self, playernum, game):
        ...
        marvelous = fontfx.shadefade(
            _("MARVELOUS"), FontTheme.Dance_step_judgment, 4, [tx, 40], [224, 224, 224]
        )

        tx = FontTheme.Dance_step_judgment.size(_("PERFECT"))[0] + 4
        perfect = fontfx.shadefade(
            _("PERFECT"), FontTheme.Dance_step_judgment, 4, [tx, 40], [224, 224, 32]
        )

        tx = FontTheme.Dance_step_judgment.size(_("GREAT"))[0] + 4
        great = fontfx.shadefade(
            _("GREAT"), FontTheme.Dance_step_judgment, 4, [tx, 40], [32, 224, 32]
        )

        tx = FontTheme.Dance_step_judgment.size(_("OKAY"))[0] + 4
        okay = fontfx.shadefade(
            _("OKAY"), FontTheme.Dance_step_judgment, 4, [tx, 40], [32, 32, 224]
        )
        ...
        miss = fontfx.shadefade(
            _("MISS"), FontTheme.Dance_step_judgment, 4, [tx, 40], [224, 32, 32]
        )
...
class Player:
    def __init__(self, pid, config, songconf, game):
        self.theme = GFXTheme(mainconfig.get("%s-theme" % game.theme, "default"), pid, game)
        self.pid = pid
        self.failed = False
        self.escaped = False
        self.states = {}
        ...
        if self.scrollstyle == 2:
            self.top = 240 - game.width // 2
        elif self.scrollstyle == 1:
            self.top = 352
        else:
            self.top = 64
        ...
        if self.game.double:
            ...
            if self.transform == 1:
                ...
                self.steps = [
                    ...
                    steps.Steps(song, diff, self, self.pid * 2, lyrics, self.game.name, offset),
                ]
            else:
                self.steps = [
                    steps.Steps(song, diff, self, self.pid * 2, lyrics, self.game.name, offset),
            ...
            args = (self.pid, self.bpm, diff, count, total_holds, self.steps[0].feet)
            for listener in self.listeners:
                listener.set_song(*args)

        else:
            self.holding = [-1] * len(self.game.dirs)
            self.steps = steps.Steps(song, diff, self, self.pid, lyrics, self.game.name, offset)
    ...
    def start_song(self):
        ...
        if mainconfig["showcombo"]:
            self.text_group.add(self.combos)
        ...
        if self.game.double:
            ...
            for i in range(2):
                ...
                for d in self.game.dirs:
                    ...
                    if not self.dark:
                        self.toparr[i][d].add(self.toparr_group)
            # Draw scrolling arrows first; receptor row (toparr) last so cues stay visible.
            self.sprite_groups = [
                self.arrow_group[0],
                self.arrow_group[1],
                self.fx_group,
            ...
            for d in self.game.dirs:
                ...
                if not self.dark:
                    self.toparr[d].add(self.toparr_group)
            self.sprite_groups = [
                self.arrow_group,
                self.fx_group,
                self.toparr_group,
        ...
        step_trace.log_start_song(
            self.pid,
            self.dark,
            len(self.toparr_group),
            tuple(self.game.dirs),
        )

    def get_next_events(self, song, curtime):
        if self.game.double:
            self.fx_data = [[], []]
            for i in range(2):
            ...
            self.fx_data = []
            self._get_next_events(
                song, self.arrow_group, self.arrows, self.steps, self.judge, curtime
            )

    def _get_next_events(self, song, arrow_grp, arrow_gfx, steps, judge, curtime):
        evt = steps.get_events(curtime)
        if evt is not None:
            ...
            for ev in events:
                if ev.feet:
                    for dir, num in zip(self.game.dirs, ev.feet):
                        if num & 1:
                            judge.handle_arrow(dir, ev.when, num & 4)
    ...
    def check_sprites(self, curtime, curbeat, arrows, steps, fx_data, judge):
        ...
        for d in misses:
            for listener in self.listeners:
                listener.stepped(self.pid, d, curtime, -1, "M", self.combos.combo)
    ...
    def should_hold(self, steps, direction, curtime):
        for i, hold_info in enumerate(steps.holdinfo):
            if hold_info[0] == self.game.dirs.index(direction):
                if (curtime - 15.0 / steps.playingbpm > hold_info[1]) and (curtime < hold_info[2]):
                    return i
    ...
    def handle_keydown(self, ev, time):
        ...
        if self.game.double:
            ...
            for listener in self.listeners:
                listener.stepped(ev[0], dir, time, etime, rating, self.combos.combo)
    ...
    def check_bpm_change(self, pid, time, steps, judge):
        ...
        if newbpm != self.bpm:
            self.bpm = newbpm
            for listener in self.listeners:
                listener.change_bpm(pid, time, newbpm)
    ...
    def game_loop(self, time, screen):
        if self.game.double:
            for i in range(2):
                ...
                self.check_bpm_change(self.pid * 2 + i, time, self.steps[i], self.judge[i])
                self.check_sprites(
                    time,
                    cur_beat,
                    self.arrow_group[i],
            ...
            self.check_sprites(
                time, cur_beat, self.arrow_group, self.steps, self.fx_data, self.judge
            )
        ...
        n_scroll = (
            len(self.arrow_group[0]) + len(self.arrow_group[1])
            if self.game.double
            else len(self.arrow_group)
        )
        first_scroll_rect = None
        if step_trace.enabled():
            if self.game.double:
                for ag in self.arrow_group:
                    if len(ag) > 0:
                        r = ag.sprites()[0].rect
                        first_scroll_rect = (r.left, r.top, r.width, r.height)
                        break
            elif len(self.arrow_group) > 0:
                r = self.arrow_group.sprites()[0].rect
                first_scroll_rect = (r.left, r.top, r.width, r.height)
        step_trace.log_draw(
            self.pid,
            time,
