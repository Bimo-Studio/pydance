...
class GenericFile:
    ...
    def find_files(self, formats):
        ...
        files = [os.path.join(dir, f) for f in os.listdir(dir) if f.lower()[-3:] in formats]
    ...
    def parse_time(self, string):
        ...
        if ":" in string:
            ...
            if len(parts) == 2:
                time = 60 * int(parts[0]) + float(parts[1])
            ...
            if string.count(".") == 2:  # M.SS.CS
                ...
                time = float(string)  # seconds in a float value

        else:
            time = int(string) / 1000.0  # no punctuation means in milliseconds
        return offset + time
...
class DanceFile(GenericFile):
    ...
    def __init__(self, filename, need_steps):
        ...
        with open(filename) as f:
            for line in f:
                ...
                if line == "" or line[0] == "#":
                    ...
                    state = DanceFile.WAITING
                elif state == DanceFile.STEPS and not need_steps:
                    pass
                else:
                    state = parsers[state](line, state_data)
        ...
        if "bpmdisplay" in self.info:
            if self.info["bpmdisplay"] == "*":
                self.info["bpmdisplay"] = [-1]
            else:
                self.info["bpmdisplay"] = [float(i) for i in self.info["bpmdisplay"].split()]

        self.resolve_files_sanely()
        self._fallback_audio_in_song_dir()
        self._infer_mix_from_path()

    def _fallback_audio_in_song_dir(self):
        """If music is missing or unresolved, pick ogg/mp3/wav in the chart directory."""
        fn = self.info.get("filename")
        if fn and os.path.isfile(fn):
            return
        by_ext = {"ogg": [], "mp3": [], "wav": []}
        for ext in ("ogg", "mp3", "wav"):
            by_ext[ext].extend(self.find_files([ext]))
        if not any(by_ext.values()):
            return
        stem = os.path.splitext(os.path.basename(self.filename))[0].lower()
        for ext in ("ogg", "mp3", "wav"):
            for path in by_ext[ext]:
                if os.path.splitext(os.path.basename(path))[0].lower() == stem:
                    self.info["filename"] = path
                    return
        for ext in ("ogg", "mp3", "wav"):
            if by_ext[ext]:
                self.info["filename"] = by_ext[ext][0]
                return

    def _infer_mix_from_path(self):
        """Set mix from folder layout under songdir when not in the chart (one-folder-per-song)."""
        if self.info.get("mix"):
            return
        dance_dir = os.path.abspath(os.path.dirname(os.path.abspath(self.filename)))
        for raw in mainconfig["songdir"].split(os.pathsep):
            root = os.path.abspath(os.path.expanduser(raw.strip()))
            if not root:
                continue
            if dance_dir != root and not dance_dir.startswith(root + os.sep):
                continue
            rel = os.path.relpath(dance_dir, root)
            if rel in (".", os.curdir) or rel.startswith(os.pardir + os.sep):
                continue
            parts = [p for p in rel.split(os.sep) if p and p not in (".", os.pardir)]
            if parts:
                self.info["mix"] = parts[0]
            return
    ...
    def parse_steps(self, line, data):
        ...
        if data[0] in games.COUPLE:
            ...
            if parts[0] in ["B", "W", "S", "D"]:
                ...
                steps.extend((int(parts[1]), " ".join(parts[2:])))
    ...
    def parse_description(self, line, data):
        ...
        elif self.description[-1] is None:
            self.description[-1] = line
        else:
            self.description[-1] += " " + line
...
class MSDFile(GenericFile):
    def __init__(self, filename, need_steps):
        ...
        for line in f:
            ...
            if len(line) == 0:
                ...
                lines.append(line[1:])  # A new tag
            else:
                lines[-1] += line

        for i, line in enumerate(lines):
            while line[-1] == ";":
                line = line[:-1]  # Some lines have two ;s.
            lines[i] = line.split(":")
    ...
    def parse_lyrics(self, filename):
        ...
        for line in f:
            line = line.strip()
            if line[1:7] == "offset":
                offset = float(line[8:-1]) / 1000.0
            if len(line) > 2 and line[1] in "0123456789":
                time = self.parse_time(line[1 : line.index("]")]) + offset
                lyr = line[line.index("]") + 1 :].split("|")
                lyr.reverse()
                self.lyrics.extend(
                    [(time, i + 1, line) for i, line in enumerate(lyr) if line != ""]
                )
    ...
    def find_winfname(self):
        ...
        if fpathparts[0] == ".":
            fpathparts = fpathparts[1:]
        if fpathparts[0].lower() == "songs":
            fpathparts = fpathparts[1:]
        ...
        for fpathpart in fpathparts:
            ...
            fpaths = [
                os.path.join(fpath, matched_part)
                for fpath in fpaths
    ...
    def find_files_sanely(self):
        if not (
            os.path.exists(self.info.get("banner", ""))
            and os.path.exists(self.info.get("background", ""))
        ):
            ...
            if len(audio) > 0:
                for fn in audio:
                    if fn.lower()[-3:] == "ogg":
                        ...
                        fn.lower()[-3:] in ["mp3", "wav"]
                        and self.info.get("filename", "").lower()[-3:] != "ogg"
                    ):
                        self.info["filename"] = fn
    ...
    def create_3panel_steps(self):
        if ("6PANEL" in self.difficulty) and ("BEGINNER" in self.difficulty["6PANEL"]):
            self.difficulty["3PANEL"] = {"BEGINNER": self.difficulty["6PANEL"]["BEGINNER"]}
            self.steps["3PANEL"] = {}
            if self._need_steps:
                steps = []
                for s in self.steps["6PANEL"]["BEGINNER"]:
                    if isinstance(s[0], float):
                        steps.append([s[0], s[2], s[3], s[5]])
                    else:
                        steps.append(s)
...
class DWIFile(MSDFile):
    ...
    def __init__(self, filename, need_steps):
        ...
        for parts in lines:
            ...
            if parts[0] == "GAP":
                self.info["gap"] = -int(float(rest))
            # If filename given, save it, and let MSDFile.find_winfname() have a stab at it.
            elif parts[0] == "FILE":
                self.info["filename"] = rest
            elif parts[0] == "TITLE":
                self.info["title"] = rest
            elif parts[0] == "ARTIST":
                self.info["artist"] = rest
            elif parts[0] == "MD5":
                self.info["md5sum"] = rest
                ...
                if "preview" in self.info:
                    ...
                    self.info["preview"] = [self.parse_time(rest), 10]
            elif parts[0] == "SAMPLELENGTH":
                if "preview" in self.info:
                    self.info["preview"][1] = self.parse_time(rest)
                else:
                    self.info["preview"] = [45, self.parse_time(rest)]
                ...
                self.bpms = [
                    ...
                    for beat, bpm in [change.split("=") for change in rest.split(",")]
                ]
            elif parts[0] == "FREEZE":
                rest = rest.replace(" ", "")
                self.freezes = [
                    (float(beat), float(wait) / 1000)
                    for beat, wait in [change.split("=") for change in rest.split(",")]
                ]
    ...
    def parse_steps(self, mode, diff, steps):
        ...
        while len(steps) != 0:
            if steps[0] in DWIFile.modes:
                step_type = DWIFile.modes[steps.pop(0)]
                ...
                tomerge = []
                while steps[0] != ">":
                    tomerge.append(steps.pop(0))
                steps.pop(0)
                steplist.append([step_type] + self.parse_merge(tomerge, dwifile_steps))
            else:
                steps.pop(0)

        if mode not in games.COUPLE:
            self.steps[mode][diff] = steplist
        else:
            if self.steps[mode].get(diff) is None:
...
class SMFile(MSDFile):
    ...
    def __init__(self, filename, need_steps):
        ...
        for parts in lines:
            ...
            if parts[0] == "OFFSET":
                self.info["gap"] = float(parts[1]) * 1000
            elif parts[0] == "TITLE":
                self.info["title"] = rest
            elif parts[0] == "SUBTITLE":
                self.info["subtitle"] = rest
            elif parts[0] == "ARTIST":
                self.info["artist"] = rest
            elif parts[0] == "CREDIT":
                self.info["author"] = rest
            elif parts[0] == "MUSIC":
                self.info["filename"] = rest
            elif parts[0] == "BANNER":
                self.info["banner"] = rest
            elif parts[0] == "BACKGROUND":
                self.info["background"] = rest
            elif parts[0] == "MD5":
                self.info["md5sum"] = parts[1]
            elif parts[0] == "CDTITLE":
                self.info["cdtitle"] = self.find_cdtitle(rest)
            elif parts[0] == "DISPLAYBPM":
                if rest != "*":
                ...
                if "preview" in self.info:
                    ...
                    self.info["preview"] = [float(parts[1]), 10]
            elif parts[0] == "SAMPLELENGTH":
                if "preview" in self.info:
                    self.info["preview"][1] = float(parts[1])
                else:
                    self.info["preview"] = [45, float(parts[1])]
            elif parts[0] == "BPMS":
                rest = rest.replace(" ", "")
                self.bpms = [
                    (float(beat), float(bpm))
                ...
                if parts[1] in SMFile.gametypes:
                    ...
                    if parts[2] == "":
                        parts[2] = parts[3]
                    if parts[1] == "ez2-single-hard":
                        parts[2] = "HARD: " + parts[2]
    ...
    def parse_steps(self, steps, gametype):
        ...
        for measure in measures:
            ...
            if notetype != 0:
                ...
                beat += 4.0  # This was an empty measure
            ...
            while len(measure) != 0:
                ...
                if gametype in games.COUPLE:
                    ...
                    step1.extend(
                        [SMFile.step[int(s.translate(ZERO_ALPHA))] for s in sd[0 : count // 2]]
                    )
                    step2.extend(
                        [SMFile.step[int(s.translate(ZERO_ALPHA))] for s in sd[count // 2 :]]
                    )
...
class KSFFile(MSDFile):
    def __init__(self, filename, need_steps):
        ...
        for fn in os.listdir(path):
            ...
            fn_lower = fn.lower()
            if fn_lower[-3:] == "ksf":
                self.parse_ksf(fullname)
            elif fn_lower[:4] == "disc":
                self.info["banner"] = fullname
            elif fn_lower[:5] == "intro":
                self.info["preview"] = fullname
        ...
        try:
            ...
            self.info["artist"] = self.info["artist"]
        except LookupError:
            pass  # Encoding support not available.
            ...
            if line[0] == "#":
                ...
                if parts[0] == "TITLE" and parts[1] != "":
                    ...
                    if len(pts) == 3:
                        ...
                        self.info["artist"], self.info["title"] = pts
                    elif len(pts) == 1:
                        self.info["title"] = pts[0]
                elif parts[0] == "BPM":
                    self.info["bpm"] = float(parts[1])
                elif parts[0] == "TICKCOUNT":
                    note_type = 4.0 / float(parts[1])
                elif parts[0] == "STARTTIME":
                    self.info["gap"] = -float(parts[1]) * 10
            elif line[0] == "2":
                break
...
def sorted_diff_list(difflist):
    ...
    # Convert cmp to key for Python 3
    from functools import cmp_to_key

    def cmp_func(a, b):
        diff_cmp = difflist[a] - difflist[b]
        if diff_cmp != 0:
            return diff_cmp
        return util.difficulty_sort(a, b)

    keys.sort(key=cmp_to_key(cmp_func))
    return keys
...
class SongItem:
    ...
    def __init__(self, filename, need_steps=True):
        ...
        # Sanity checks (filename optional: charts may ship without audio in-repo)
        for k in ["bpm"]:
            if k not in self.info:
                raise RuntimeError(filename + _(" is missing: ") + k)
        self.info.setdefault("filename", None)
        ...
        # Default values
        for k in ["background", "banner", "revision", "md5sum", "movie", "cdtitle"]:
            self.info.setdefault(k, None)

        for k in SongItem.defaults:
            self.info.setdefault(k, SongItem.defaults[k])

        # Charts placed directly in a songdir root (not in a subfolder) do not get a path-based
        # mix from DanceFile._infer_mix_from_path, so they all stayed "No Mix" — one folder and it
        # looks like there are no mixes. Use the chart stem as mix name so "Sort by mix" lists one
        # folder per loose file (same idea as one-folder-per-song packs).
        if self.info.get("mix") in (None, "", SongItem.defaults["mix"]):
            dance_dir = os.path.abspath(os.path.dirname(os.path.abspath(filename)))
            for raw in mainconfig["songdir"].split(os.pathsep):
                root = os.path.abspath(os.path.expanduser(raw.strip()))
        ...
        if self.info["mix"] == "Unknown":
            self.info["mix"] = "No Mix"
        ...
        if mainconfig["autogen"]:
            ...
            for game in games.GAMES:
                if game in self.difficulty:
                    continue  # This mode is defined already.
